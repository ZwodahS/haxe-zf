...
/**
	...
	1. Allow for center to be tiled instead of scaled.
	2. Replaced tileBorders with tiling
**/
class ScaleGrid extends h2d.TileGroup {
	...
	**/
	...
	@:deprecated
	public var tileBorders(get, set): Bool;
	...
	**/
	public var tiling(default, set): Bool;
	...
	**/
	...
	function set_tileBorders(b) {
		return this.tiling = b;
	}

	function get_tileBorders() {
		return this.tiling;
	}

	function set_tiling(b: Bool) {
		if (this.tiling == b) return b;
		this.tiling = b;
		clear();
		return b;
	}
	...
	function updateContent() {
		...
		if (!this.tiling) {
			...
			final rw = Std.int(innerWidth / unscaledInnerTileWidth);
			for (x in 0...rw) {
				// top
				var t = tile.sub(bl, 0, innerTileWidth, bt);
			...
			final rh = Std.int(innerHeight / unscaledInnerTileHeight);
			for (y in 0...rh) {
				// left
				var t = tile.sub(0, bt, bl, innerTileHeight);
			...
			for (y in 0...rh) {
				for (x in 0...rw) {
					var t = tile.sub(bl, bt, innerTileWidth, innerTileHeight);
					t.scaleToSize(unscaledInnerTileWidth, unscaledInnerTileHeight);
					content.addColor(unscaledBl + x * unscaledInnerTileWidth,
						unscaledBt + y * unscaledInnerTileHeight, curColor, t);
				}
				// we can put in the last column with partial tile
				if (dx > 0) {
					final t = tile.sub(bl, bt, dx / invScaleX, innerTileHeight);
					t.scaleToSize(dx, unscaledInnerTileHeight);
					content.addColor(unscaledBl + rw * unscaledInnerTileWidth,
						unscaledBt + y * unscaledInnerTileHeight, curColor, t);
				}
			}
			...
			if (dy > 0) {
				for (x in 0...rw) {
					var t = tile.sub(bl, bt, innerTileWidth, dy / invScaleY);
					t.scaleToSize(unscaledInnerTileWidth, dy);
					content.addColor(unscaledBl + x * unscaledInnerTileWidth,
						unscaledBt + rh * unscaledInnerTileHeight, curColor, t);
				}
				if (dx > 0) {
					final t = tile.sub(bl, bt, dx / invScaleX, dy / invScaleY);
					t.scaleToSize(dx, dy);
					content.addColor(unscaledBl + rw * unscaledInnerTileWidth,
						unscaledBt + rh * unscaledInnerTileHeight, curColor, t);
				}
