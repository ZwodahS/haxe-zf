...
private class TreeNode extends UIElement {
	...
	public function new(inspector: OverlayInspector, level: Int, name: String, data: Dynamic) {
		super();
		this.level = level;
		this.data = data;
		this.displayName = name;
		this.text = new HtmlText(inspector.font);
		this.text.text = formatDisplay();
		this.text.textColor = inspector.conf.textColor;
...
class OverlayInspector extends h2d.Object {
	...
	public var treeNodes: Array<TreeNode>;
	...
	function inspectObjects(objects: Array<{name: String, object: Dynamic}>) {
		...
		for (o in objects) {
			final node = new TreeNode(this, 0, o.name, o.object);
			node.addOnClickListener("OverlayInspector", (_) -> {
				onNodeClick(node);
			});
