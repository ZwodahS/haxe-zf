...
class Console extends h2d.Object {
	...
	**/
	public function new(font: h2d.Font, ?parent, g: Game) {
		super(parent);
		this.g = g;
	...
	**/
	...
	function onEvent(e: hxd.Event) {
		switch (e.kind) {
			...
			case ETextInput:
				if (e.charCode == shortKeyChar && !bg.visible) show();
			default:
		}
	}
	...
	**/
	...
	function getCommandSuggestion(cmd: String): String {
		...
		if (tokenizedCmds.length == 1) {
			...
			if (hadShortKey && closestCommand != "") closestCommand = String.fromCharCode(shortKeyChar)
				+ closestCommand;
